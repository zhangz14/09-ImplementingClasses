...
class Point(object):
    ...
    def move_to(self, x, y):
        self.totald = self.totald + ((x - self.x) ** 2 + (y - self.y) ** 2) ** (1 / 2)
        self.x = x
        self.y = y
        self.moves = self.moves + 1

    def move_by(self, dx, dy):
        self.totald = self.totald + (dx ** 2 + dy ** 2) ** (1 / 2)
        self.x = self.x + dx
        self.y = self.y + dy
        self.moves = self.moves + 1
    ...
    def get_distance_from(self, point):
        d = ((point.x - self.x) ** 2 + (point.y - self.y) ** 2) ** (1 / 2)
        return d

    def get_distance_from_start(self):
        dfs = ((self.x - self.startx) ** 2 + (self.y - self.starty) ** 2) ** (1 / 2)
        return dfs
